...
def user_detail(username):
    ...
    user = User.objects(username=username).first()
    if user is None:
        return render_template('user_detail.html', error = "User does not exist", image = None, username=username)
    else:
        return render_template('user_detail.html', error = None, image = get_b64_img(username), username=username, reviews = Review.objects(commenter=user))
